...
if (!effect) throw '*[❗𝐈𝐍𝐅𝐎❗] ¿𝐂𝐨𝐦𝐨 𝐮𝐬𝐚𝐫 𝐞𝐬𝐭𝐞 𝐜𝐨𝐦𝐚𝐧𝐝𝐨?*\n—◉ _#logo (efecto) (texto)_\n*𝐄𝐣𝐞𝐦𝐩𝐥𝐨:*\n—◉ _#logo 3d-deep-sea-metal The King Bot_\n\n*[❗] 𝐒𝐢 𝐥𝐞𝐬 𝐝𝐢𝐜𝐞 𝐪𝐮𝐞 𝐟𝐚𝐥𝐭𝐚 𝐮𝐧 𝐭𝐞𝐱𝐭𝐨 𝐞𝐥 𝐮𝐬𝐨 𝐬𝐞𝐫𝐢𝐚:*\n—◉ _#logo (efecto) (texto1|texto2)_\n*𝐄𝐣𝐞𝐦𝐩𝐥𝐨:*\n—◉ _#logo a-glitch The King Bot|By Matias_\n\n*<𝗟𝗜𝗦𝗧𝗔𝗦 𝗗𝗘 𝗘𝗙𝗘𝗖𝗧𝗢𝗦/>*\n\n° ඬ⃟📝 #logo ' + effects.map(v => v.title).join('\n° ඬ⃟📝 #logo ')
effect = effect.toLowerCase()
if (!effects.find(v => (new RegExp(v.title, 'gi')).test(effect))) throw `*[❗𝐈𝐍𝐅𝐎❗] 𝐄𝐥 𝐞𝐟𝐞𝐜𝐭𝐨 ${effect} 𝐧𝐨 𝐞𝐬𝐭𝐚 𝐞𝐧 𝐥𝐚 𝐥𝐢𝐬𝐭𝐚 𝐝𝐞 𝐞𝐟𝐞𝐜𝐭𝐨𝐬*`
let text = txt.replace(new RegExp(effect, 'gi'), '').trimStart()
if (text.includes(split)) text = text.split(split)
text = Array.isArray(text) ? text : [text]
let res = await textpro(effect, ...text)
if (typeof res == 'number') throw res == -1 ? `*[❗𝐈𝐍𝐅𝐎❗] 𝐄𝐥 𝐞𝐟𝐞𝐜𝐭𝐨 ${effect} 𝐧𝐨 𝐞𝐬𝐭𝐚 𝐞𝐧 𝐥𝐚 𝐥𝐢𝐬𝐭𝐚 𝐝𝐞 𝐞𝐟𝐞𝐜𝐭𝐨𝐬*` : `*[❗𝐈𝐍𝐅𝐎❗] 𝐄𝐥 𝐮𝐬𝐨 𝐜𝐨𝐫𝐫𝐞𝐜𝐭𝐨 𝐝𝐞𝐥 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 𝐞𝐬 ${usedPrefix + command} ${effect} ${new Array(res).fill('texto').map((v, i) => v + (i ? i + 1 : '')).join('|')}*`
let result = await axios.get(res, {
responseType: 'arraybuffer'
})
await conn.sendFile(m.chat, result.data, 'Error.jpg', `*𝐓𝐨𝐦𝐚 𝐭𝐮 𝐢𝐦𝐚𝐠𝐞𝐧 𝐩𝐞𝐫𝐬𝐨𝐧𝐚𝐥𝐢𝐳𝐚𝐝𝐨!!!*\n*𝐄𝐟𝐞𝐜𝐭𝐨: ${effect}*`, m)
}
handler.help = ['logos']
handler.tags = ['nulis']
